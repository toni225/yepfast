...
import { checkCircleInMarker } from "./ComputeDistance";
...
const MapDisplay = ({ data = [], page, markedLocation }) => {
  ...
  const CDNURL =
    "https://evrqsaavaohqlopnfgtq.supabase.co/storage/v1/object/public/images/";
  ...
  const getParkingImage = async (username, parkingName) => {
    userService.getImageParking(username, parkingName).then((res) => {
        ...
        <div style={{ height: "90vh", position: "relative" }}>
          <Map
            ...
            center={
              state === null
                ? mapCenter
                : { lat: parseFloat(state?.lat), lng: parseFloat(state?.lng) }
            }
            onClick={(e) => {
              if (page === "CreateParking") {
                setMarker({
          ...
          >
              ...
              <div key={parking.ParkingID}>
                <AdvancedMarker
                  ...
                  position={{
                    ...
                    lng: parseFloat(parking.ParkingLocation.Lng),
                  }}
                >
                  <Pin
                    background={"#22ccff"}
                  ...
                  {openInfoWindow && selectedMarker === parking.ParkingID && (
                    ...
                    >
                      <div id="container" className="m-[10px] relative">
                        <img
                          className="w-[262px] h-[131px] rounded-xl border border-black"
                          rel={"image"}
                          src={`${CDNURL}${parking.username}/${parking.ParkingName}/${image}`}
                        ></img>
                        <div className="flex flex-col justify-between items-center max-w-[250px]">
                          <div className="bg-VO-Secondary mt-[-20px] py-[5px] shadow-my-shadow px-[10px] rounded-full text-lg">
                            {parking.ParkingName}
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 justify-items-stretch text-center my-3">
                          ...
                          <div className="flex flex-col justify-between items-center">
                            ...
                            <p>Contact Number</p>
                            <p>{parking.username.ContactNum}</p>
                          </div>
                        </div>
            ...
            {doDirections && (
              <Directions origin={origin} directions={directions} />
            )}
